...
public class Parser {
	...
	public static boolean addKeyWord (String word) {
	...
	public static ArrayList<SmsRecord> parse(SMS sms) {
		...
		keyWords.add(goldenGate); keyWords.add(otrabotalo); keyWords.add(nochyuUpalo);
		keyWords.add(AbonToday); keyWords.add(viruchka); keyWords.add(operativniy);
		keyWords.add(SEND_IMSI); keyWords.add(vcheraUpalo); keyWords.add(padalo);
		...
		for (int i = 0; i < arrayPhrases.length; i++) {
			...
			//value
			//arrayPhrases[i].substring(arrayPhrases[i].indexOf(":"), arrayPhrases[i].length());
			
			for (int j = 0; j < keyWords.size() && !added; j++) {
				
				if (arrayPhrases[i].indexOf(":") != -1 && arrayPhrases[i].substring(0, arrayPhrases[i].indexOf(":")).equals(keyWords.get(j))) {
					
					SmsRecord newRecord = new SmsRecord(sms.getId(), sms.getDate(), keyWords.get(j), arrayPhrases[i].substring(arrayPhrases[i].indexOf(":") + 2, arrayPhrases[i].length()));
